...
module.exports = syrup.serial()
  ...
  .define(function (options, wda) {
    ...
    plugin.start = async function () {
      ...
      log.info("wdaPort:%d mjpegPort:%d serial:%s",options.wdaPort, options.mjpegPort, options.serial)
    ...
    plugin.startIproxy = function (localPort, remotePort) {
      log.info("start tidevice relay with params:%d %d %s", localPort, remotePort, options.serial)
      // pro = new SubpPocess("iproxy", [localPort, remotePort, "-l", "-s", "127.0.0.1", "-u", options.serial])
      log.info("tidevice -u %s relay %d %d", localPort, remotePort, options.serial)
      pro = new SubpPocess("tidevice", ["-u", options.serial, "relay", localPort,remotePort])
      pro.start();
      pro.on("exit", (code, signal) => {
        log.info("tidevice exit with code :%d", code)
    ...
    plugin.startWda = function () {
      ...
      wdaPro = new SubpPocess("tidevice", ["-u", options.serial,
        "xctest", "-B", "com.facebook.*.xctrunner"])
      wdaPro.start()
