  ...
  .define(function (options, router, push, group) {
    ...
    plugin.start = function (filters) {
      return group.get().then(function (group) {
        ...
        plugin.stop()
          .then(function () {
            log.info('Starting tidevice syslog')
            var param = ['-u', options.serial, 'syslog']
            if (filters.length) {
              param = ['-u', options.serial, 'syslog', '|grep', filters[0]]
            }
            // activeLogcat = new SubpPocess("idevicesyslog",param)
            activeLogcat = new SubpPocess("tidevice", param)
            activeLogcat.start()
