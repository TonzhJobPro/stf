...
module.exports = syrup.serial()
  ...
  .define(function(options, display, screenOptions) {
    ...
    FrameProducer.prototype._startService = function() {
      log.info('Launching screen service - socket port:'+screenOptions.devicePort)
      this.socket = net.connect({
        port: screenOptions.devicePort
      })
      this.socket.write("start send ios jpeg")
      log.info('start send ios jpeg')
    }

    FrameProducer.prototype._stop = function() {
      log.info('stop send ios jpeg')
          ...
          function send(message, options) {
            return new Promise(function(resolve, reject) {
              ...
              case WebSocket.OPEN:
                ...
                log.info('ios send image data to web')
                //index++
                //if(index%5==0){
                //    break
                //}
